## 2026-08-03 全项目 Flatpickr 时间选择器中文化

- [x] 增加失败审计测试，锁定全部 DateTimePicker/DatePicker 页面覆盖。
- [x] 新增列表、编辑和批量编辑共享中文日期模板。
- [x] 接入所有遗漏视图和自定义模板，并禁用新增自动填时行为。
- [x] 运行项目级日期控件审计、相关回归、编译和差异检查。

## 2026-08-03 故障列表时间选择器中文化

- [x] 增加失败测试，锁定列表页加载中文 locale 且不自动写入筛选时间。
- [x] 参数化现有 Flatpickr 中文模板的自动填时行为。
- [x] 在故障列表模板中启用中文 locale 并禁用自动填时。
- [x] 运行定向回归、Python 编译和差异检查。

## 2026-08-03 故障列表增加裸纤业务中断筛选

- [x] 增加失败回归测试，锁定筛选字段、裸纤中断口径和表单位置。
- [x] 在 `OtnFaultFilterSet` 增加“造成裸纤业务中断”布尔筛选，支持数量大于 0 与等于 0。
- [x] 在 `OtnFaultFilterForm` 增加“是/否”下拉框，并放在故障起始时间和处理派发时间之间。
- [x] 运行定向测试、相邻回归测试、Python 编译与差异检查。

## 2026-08-03 故障明细显示裸纤业务中断数量

- [x] 增加失败回归测试，锁定中断裸纤业务去重聚合、字段返回和 0 显示横线。
- [x] 为当前故障与历史重复故障统一注解并返回 `bare_fiber_impact_count`。
- [x] 在物理故障和子公司故障明细增加“裸纤业务中断”列并调整列数。
- [x] 运行统计测试、Python 编译、JavaScript 语法、差异和索引检查。

## 2026-08-03 裸纤业务中断卡片下钻故障明细

- [x] 先增加失败回归测试，锁定四张卡片统一携带裸纤业务中断下钻条件。
- [x] 提取有效裸纤业务中断影响记录的共用筛选逻辑，保证统计与明细口径一致。
- [x] 扩展故障明细接口，按影响记录关联的故障 ID 去重返回故障，并支持子公司范围。
- [x] 完成四张卡片的物理故障页和子公司页下钻交互及筛选提示。
- [x] 运行定向测试、Python 编译、JavaScript 语法和差异检查。

## 2026-07-31 割接通报未关联业务文案修正

- [x] 增加未关联业务通报正文的失败回归测试，锁定“不影响业务”措辞。
- [x] 在统一正文格式化器中仅对“未关联业务”使用“不影响业务”，保持已关联业务格式不变。
- [x] 运行割接通报定向测试、Python 编译检查和差异检查。

## 2026-07-31 仪表盘故障/割接月历月份滚动

- [x] 在 `tests/test_dashboard_calendar_month_navigation.py` 先增加月份解析、跨年滚动及故障/割接未来上限的失败测试。
- [x] 在 `netbox_otnfaults/dashboard.py` 提取带类型提示的月份解析与相邻月份辅助函数，并让两个 Widget 按请求年月构造日历。
- [x] 先增加两个局部刷新路由和视图的失败测试，再在 `netbox_otnfaults/urls.py` 与独立视图模块中实现只读、登录保护的 HTML 片段端点。
- [x] 先扩充两个模板的失败测试，锁定左右侧中部热区、默认隐藏、悬停/焦点显示和达到未来上限时不可用。
- [x] 在两个日历模板中加入独立根容器与月份导航按钮，并增加共用脚本，以 `fetch()` 仅替换当前小组件。
- [x] 验证故障最多滚动到当前月，割接最多滚动到下个月，历史月份可持续向前滚动且跨年正确。
- [x] 运行两个日历定向测试、导航新增测试、Python 编译、JavaScript 语法和差异检查。

## 2026-07-31 割接 9 点/18 点通报书面化

- [x] 增加通报正文格式化回归测试，锁定固定书面模板和全部动态字段去空白行为。
- [x] 去除正文中括号、箭头及“申请”措辞，改为中文日期和独立 A/Z 端分句。
- [x] 保持租户组分组、割接数量统计及复制功能不变。
- [x] 运行割接通报定向测试、Python 编译检查和差异检查。

## 2026-07-30 割接通报按租户组便捷复制

- [x] 增加结构化租户组通报数据，支持全部分组同时展示及单组复制。
- [x] 弹窗纵向展示全部租户组，每组标题旁直接提供复制本组按钮。
- [x] 保留复制全部功能，减少用户完成复制所需操作次数。
- [x] 将关闭按钮放在弹窗底部操作区最右侧。
- [x] 运行定向测试、Python 编译和 JavaScript 语法检查。

## 2026-07-30 割接通报按裸纤租户组分组并显示割接数量

- [x] 调整回归测试，锁定裸纤业务按租户组分组。
- [x] 分组标题显示该租户组涉及的去重割接数量。
- [x] 电路业务及未设置租户组的裸纤业务使用明确的兜底分组。
- [x] 运行定向测试、Python 编译及差异检查。

## 2026-07-30 今明割接小组件生成 9 点/18 点割接通报

- [x] 增加通报生成回归测试，覆盖时间窗、待实施状态、业务名称分组与前端按钮。
- [x] 在今明割接小组件中分别查询当日 09:00 至次日 09:00、当日 18:00 至次日 18:00 的待实施割接。
- [x] 按关联裸纤/电路业务提取业务名称与业务 A/Z 站点生成正文，并按裸纤租户组汇总通报。
- [x] 增加 9点通报、18点通报按钮、通报查看弹窗及复制功能。
- [x] 运行定向测试、Python 编译检查并复核变更。
...
- [x] 在大屏割接数据中返回解析后的 `lat`、`lng` 以及 A/Z 站点 ID。
- [x] 坐标无法解析时返回空坐标且不生成地图标记，同时保留割接列表信息。
- [x] 运行态势大屏、坐标解析相关测试和 Python 编译检查。
...
- [x] 运行割接状态定向测试和 Python 编译检查。

## 2026-07-15 取消割接影响业务保存身份校验
- [x] 增加回归测试，锁定 `CutoverImpactEditView` 不再按线路主管、业务主管或超级管理员身份拦截保存。
- [x] 删除视图中的自定义 `post()` 身份校验，保存权限统一交由 NetBox 模型权限处理。
- [x] 删除割接影响业务编辑模板中失去触发入口的权限拒绝弹窗代码。
- [x] 运行定向测试、相关割接管理测试和 Python 编译检查。
...
- [x] 运行完整统计测试、JavaScript 与 Python 语法检查。
## 2026-06-19 统一光缆中断卡片与明细统计口径
- [x] 增加回归测试，锁定光缆中断区域统一携带明细作用域。
- [x] 明细接口复用卡片的光缆中断、非挂起统计口径，并仅返回当期明细行。
- [x] 下钻提示逐项展示统计范围、挂起状态、主条件和附加条件。
- [x] 运行完整统计测试、JavaScript 与 Python 语法检查。
## 2026-06-19 统一光缆中断全部指标与下钻明细口径
- [x] 审计光缆中断卡片、主要原因、光缆属性、历时频数和省份图的统计数据源与点击参数。
- [x] 增加失败回归测试，锁定三张分析图统一使用光缆中断明细范围、省份图遵循当前省份筛选。
- [x] 统一重复中断下钻为 KPI 的“当前故障存在 60 天内前序同 A/Z 端故障”口径。
- [x] 明细接口返回基于原始历时计算的服务端汇总，避免逐行四舍五入造成总历时和平均历时偏差。
- [x] 前端记录点击指标的计数、总历时、平均、超时率和分位值语义，并在下钻说明中展示指标值、明细起数、历时汇总及计算关系。
- [x] 运行完整统计测试、JavaScript 语法检查和 Python 编译检查。
## 2026-06-19 故障等级环形图增加完整下钻
- [x] 增加回归测试，锁定三个环形图扇区和圆心下钻行为。
- [x] 后端故障明细接口增加光缆中断、供电、空调与设备分组过滤。
- [x] 扇区点击按故障分组和等级过滤，圆心点击仅按故障分组过滤。
- [x] 运行统计测试、JavaScript 与 Python 语法检查。
## 2026-06-19 修复物理故障类型明细下钻
- [x] 增加回归测试，锁定物理页六类故障类型均生成分类下钻属性。
- [x] 总体情况四类故障通过 `category` 字段过滤明细。
- [x] 光缆劣化和光缆抖动通过 `category` 字段过滤明细。
- [x] 运行完整统计测试及 JavaScript 语法检查。
## 2026-06-19 修复子公司故障明细省份范围
- [x] 增加回归测试，锁定子公司明细复用标准化省份及维护单位排除规则。
- [x] 移除子公司明细对省份数据库名称的精确匹配。
- [x] 当前周期与前序重复故障统一应用子公司范围判断。
- [x] 运行完整统计测试及 JavaScript/Python 语法检查。
## 2026-06-19 恢复故障统计全部逻辑与展现
- [x] 增加回归测试，覆盖所有故障明细筛选条件和中文显示值映射。
- [x] 恢复物理故障、子公司卡片及图表下钻的完整统计逻辑。
- [x] 保持完整明细、重复分组排序、裸纤全部业务和业务明细展现逻辑。
- [x] 修复历史统计缓存键的日历年月隔离。
- [x] 为统计模型索引补充正式迁移。
- [x] 运行统计测试、迁移静态检查、JavaScript 和 Python 语法检查。

## 2026-06-19 审计 d093d50 故障统计优化提交
- [x] 对比 `d093d50^..d093d50` 的全部文件和提交意图，建立变更清单。
- [x] 审查后端统计查询、统计口径、筛选范围和接口响应是否发生行为变化。
- [x] 审查前端统计交互、排序、分页、标签切换和明细展示是否发生行为变化。
- [x] 对照后续修复提交与现有测试，确认已修复和仍残留的回归。
- [x] 输出按严重程度排序的审计结论，明确哪些修改应保留、回退或重做。

## 2026-06-19 故障统计模块：取消明细分页并恢复重复排序
- [x] 移除物理故障、子公司、裸纤业务和电路业务明细分页 UI 与前端分页状态。
- [x] 调整故障明细和业务明细接口，一次返回全部筛选结果。
- [x] 恢复物理故障和子公司完整明细缓存、局部统计及按时间/按重复排序。
- [x] 故障明细补充与当前周期关联的前序重复故障，并保持子公司范围过滤一致。
- [x] 运行统计定向测试、业务排序断言、JavaScript 和 Python 语法检查。

## 2026-06-19 故障统计模块：修复子公司有效历时明细切换
- [x] 增加回归测试，锁定子公司明细接口处理 `is_valid_duration=true`。
- [x] 后端故障明细分页接口按恢复时间或当前时间计算历时，并过滤大于 30 分钟的故障。
- [x] 运行子公司统计与故障统计定向测试、JavaScript 语法检查和 Python 语法检查。

## 2026-06-19 故障统计模块：修复业务故障明细始终为空
- [x] 增加回归测试，锁定前端使用 `bare_fiber` / `circuit` 查询业务明细，后端兼容中文显示值。
- [x] 修复业务明细接口的业务类型参数映射，避免使用中文显示值直接过滤数据库枚举字段。
- [x] 运行故障统计定向测试、JavaScript 语法检查和 Python 语法检查。

## 2026-06-19 故障统计模块：修复裸纤业务范围切换
- [x] 增加回归测试，锁定默认仅返回发生故障业务、显式参数按需返回全部裸纤业务。
- [x] 后端业务统计接口支持 `include_all_bare_fiber=1`，仅在显式请求时初始化全部裸纤业务。
- [x] 前端切换“全部业务”时按需请求全量数据，并在同一统计条件下复用缓存。
- [x] 运行统计定向测试、JavaScript 语法检查和 Python 编译检查。

## 2026-06-14 故障统计模块：修复隐藏 Tab 下 ECharts 运行月历图 coord 报错
- [x] 调整变量声明顺序，将 annualYear、currentMonth、isCurrentYear 等前置。
- [x] 根据当前年份和月份，将原始折线数据 durationValues 拆分为已发生与未发生两个数组。
- [x] 在 series 中将单一的“故障时长”折线系列拆分为双折线系列，实现今年已过去月份与未来月份的多段颜色无缝拼接。
...
- [x] 增加回归测试，锁定子公司故障详情提供按时间排序 and 按重复排序选项。
- [x] 在子公司详情卡片头部增加独立排序按钮组，保持不影响物理故障详情排序状态。
- [x] 调整前端详情渲染逻辑，让子公司详情按时间排序时隐藏前续重复记录，按重复排序时展示重复分组。
- [x] 运行子公司统计定向测试、JS 语法检查和 Python 编译检查。


## 2026-06-12 故障统计模块：统一子公司排除维护单位口径
- [x] 增加回归测试，锁定子公司统计、子公司明细和绩效卡片都通过统一的后端子公司口径过滤。
- [x] 在 `statistics_views.py` 中封装子公司统计纳入条件，集中应用 `EXCLUDED_HANDLING_UNITS`。
- [x] 调整故障统计 API 返回给前端的子公司明细数据，避免前端从全量物理明细二次筛选导致漏排除。
- [x] 运行子公司统计定向测试和 Python 编译检查。

## 2026-06-12 故障统计模块：子公司绩效卡片增加年度指标
- [x] 增加回归测试，锁定绩效卡片年度统计 payload 包含裸纤业务、光缆中断和供电故障指标，并按光缆中断千公里次数排序。
- [x] 增加前端源码回归测试，锁定卡片名称下方、月度图上方渲染年度指标块。
- [x] 后端补充年度统计：裸纤业务中断影响、光缆中断不含挂起、供电故障与设备脱管。
- [x] 前端新增现代紧凑指标布局，并保持卡片原有图标、名称、月度图和中断日历。
- [x] 运行相关测试和 JS 语法检查。

## 2026-06-12 故障统计模块：子公司绩效月度图增加千公里切换
- [x] 增加回归测试，锁定绩效卡片 `monthly_stats` 输出原始值与千公里字段。
- [x] 增加回归测试，锁定子公司绩效页起数/时长图提供“原始值/千公里”切换且默认千公里。
- [x] 调整后端绩效卡片月度 payload，补充 `count_per_1000km` 与 `duration_per_1000km`。
- [x] 调整前端模板和脚本，按切换项渲染绩效卡片月度图。
- [x] 运行相关测试和 JS 语法检查。

## 2026-06-12 故障统计模块：简化子公司绩效卡片
- [x] 增加前端源码回归测试，锁定绩效 Tab 名称为“子公司绩效”，并锁定绩效卡片只展示公司图标、名称、底部历时/次数图表与中断日历。
- [x] 调整 `statistics_dashboard.html` 中绩效 Tab 文案。
- [x] 精简 `statistics_dashboard.js` 中绩效卡片渲染模板，保留后端统计 payload 和前端日历初始化逻辑。
- [x] 运行相关测试和语法检查。

## 2026-06-12 故障统计模块：物理页概览区顺序对齐子公司页
- [x] 增加回归测试，锁定物理故障 Tab 中“总体情况、裸纤业务中断情况、光缆中断情况”的展示顺序。
- [x] 调整 `statistics_dashboard.html` 中物理故障 Tab 的 section 顺序，保持卡片内容与 DOM ID 不变。
- [x] 运行相关模板测试，确认顺序调整未影响现有概览区域。

## 2026-06-12 故障统计模块：子公司页增加裸纤业务中断情况
- [x] 后端统计口径：复用 `_compute_bare_fiber_interruption_overview()`，为子公司统计 payload 增加 `bare_fiber_interruption`，按六省子公司归属与排除规则过滤。
- [x] 前端模板：在子公司（省份）故障统计 Tab 增加“裸纤业务中断情况”四指标卡片，使用独立 DOM ID，避免与物理故障页冲突。
- [x] 前端脚本：扩展裸纤中断渲染函数支持 ID 前缀，并在 `renderBranchCompanyOverview()` 中渲染子公司页指标及同期趋势。
- [x] 回归验证：更新 `tests/test_statistics_branch_company.py` 并运行相关测试和语法检查。
...
- [ ] 调整前端指标说明文案与相关 DOM 属性，将业务卡片的中断日历说明同步为“近三个月”。
- [ ] 运行故障统计相关自动化测试。
...
- [x] 将一张图左上角故障视图与割接显示控件改为竖向排列。
- [x] 修正右上角图层显示浮动菜单定位，避免菜单向右错位。
- [x] 运行割接控件、割接图标和故障模式 JS 语法定向测试。
## 2026-06-05 故障统计物理页全局省份过滤
- [x] 增加源码级回归测试，锁定物理故障 Tab 专属的省份多选控件、前端参数传递和后端过滤口径。
- [x] 在故障统计模板顶部控制区增加物理故障页专属省份多选，候选来自后端 `Region` 列表。
- [x] 调整统计页前端脚本，物理故障 Tab 显示省份过滤，切换省份后重新加载物理统计数据。
- [x] 调整统计 API：用所选省份过滤物理统计数据和明细，但各省故障统计图始终使用全量省份聚合。
- [x] 修复省份过滤后的上一周期对比口径，物理页总体/分类/其他指标的增减统计按同一省份过滤，子公司统计仍保持全量口径。
- [x] 运行统计页定向测试、Python 编译检查和 JavaScript 语法检查。
- [x] 修复省份多选在未选、单选和多选状态下撑高或挤压顶部工具条的问题，固定多选控件单行高度并允许工具条有序换行。
- [x] 修复省份多选 TomSelect 与原生 select/默认阴影样式叠加导致的双底线问题，统一由 TomSelect 控制单一边框。
- [x] 修复顶部周期选择 TomSelect 默认阴影导致的双底线问题，与省份控件统一为单一边框。

## 2026-06-05 子公司有效平均历时图简化
- [x] 增加源码级回归测试，锁定子公司有效平均历时图不再显示“百公里平均”切换。
- [x] 移除该图右上角指标切换控件，并让前端固定按平均历时（小时）渲染。
- [x] 限制子公司年初至今周趋势图：指标切换到有效时长时禁用百公里选项，并自动回退到原始值。
- [x] 为子公司图表增加六省固定色板，拉开山东与江西颜色差异，并让柱状图和周趋势图统一按省份取色。
- [x] 将子公司固定色板替换为指定方案：稳定蓝、暖橙、草木绿、柔紫、金黄、棕褐。
- [x] 移除子公司中断时长箱线图的百公里平均切换，固定使用原始中断时长口径。
- [x] 优化子公司年初至今周趋势图 X 轴：按周绘制数据、按月份标识刻度，并取消分类轴两端留白。
- [x] 在子公司故障统计页新增年初至今月趋势柱状图，复用周趋势指标与百公里切换，并按六省月度聚合。
## 2026-05-19 增加割接影响业务功能
- [x] 在 `models.py` 新增 `CutoverImpact` 模型，包含裸纤与电路双重业务支持，并定义防重复约束与历时方法。
- [x] 新增并检查 `CutoverImpact` 迁移，确保无冲突且不影响已有故障影响业务。
- [x] 在 `forms.py` 补齐 `CutoverTask*` 和 `CutoverImpact*` 表单，确保当前视图导入可用。
- [x] 在 `tables.py` 增加 `CutoverImpactTable` 及其对应的 Summary 表格。
- [x] 完善 `CutoverImpactFilterSet`、API serializer 和详情页模板缺失字段。
- [x] 编写 CRUD 视图（继承通用 `NetBoxModel*`），并在 `api/serializers.py`、`api/views.py` 注册接口，最终配置 `urls.py` 与 `api/urls.py`。
- [x] 在 `CutoverTaskView.get_extra_context` 中通过 `impacts` 查出受影响业务列表。
- [x] 编写前端详情与编辑模板，更新割接详情页模板加入影响业务面板及自定义分页 UI。
- [x] 补充并运行割接管理定向源代码级测试，确保本功能验收标准通过。

## 2026-05-18 割接详情页布局重整
- [x] 增加源码级回归测试，锁定详情页采用顶部摘要、响应式主网格和独立关联用户宽表。
- [x] 重构割接详情页模板，减少左右列高度失衡和表格嵌套挤压。
- [x] 保留计划时间历史列表、关联用户只读表格和 NetBox 原生 Bootstrap 风格。
- [x] 运行定向测试验证模板结构。

## 2026-05-18 割接详情页 JSON 展示优化
- [x] 增加源码级回归测试，锁定详情页计划割接时间和关联用户按维护页逻辑展示，不再直接输出 JSON。
- [x] 调整割接详情页计划时间记录为历史列表，显示次数、时间和最新标记。
- [x] 调整割接详情页关联用户为只读表格，展示租户组、业务名称、已协调和协调时间。
- [x] 运行定向测试验证模板变更。

## 2026-05-18 割接遗留 remarks 列清理
- [x] 增加源码级回归测试，锁定迁移会处理遗留 `remarks` 非空列，避免割接保存时报 NOT NULL。
- [x] 新增迁移，存在 `remarks` 列时先合并到 `comments`，再删除旧列。
- [x] 运行定向测试和迁移语法编译检查。

## 2026-05-18 割接客户审核明细空值保存
- [x] 增加源码级回归测试，锁定割接表单会将空 JSON 字段清洗为空列表，避免数据库 NOT NULL 报错。
- [x] 调整割接表单清洗逻辑，覆盖客户审核明细、计划割接时间记录和关联用户。
- [x] 运行定向测试验证变更。

## 2026-05-18 生成割接时间确认提示
- [x] 增加源码级回归测试，锁定重新生成割接时间前必须弹出确认提示，并说明会清理关联用户确认状态。
- [x] 调整割接编辑页脚本，用户取消确认时不生成新时间。
- [x] 用户确认重新生成后，清理所有关联用户的确认状态和协调时间并同步 JSON。
- [x] 运行定向测试和内联脚本语法检查验证变更。

## 2026-05-16 业务中断日历展开全年视图
- [x] 增加源码级回归测试，锁定业务中断日历默认仍展示近三个月，展开后展示本年 1 月至当前月每日中断数量。
- [x] 调整业务统计后端，为每张业务卡片同时返回默认近三个月日历和本年展开日历数据。
- [x] 调整统计页前端，在月历右下角增加展开/收回图标按钮，点击时切换日历视图且不触发卡片下钻。
- [x] 运行定向测试和语法检查验证变更。

## 2026-05-18 割接关联用户统一选择
- [x] 增加源码级回归测试，锁定割接编辑页使用“设置关联用户”弹窗从裸纤业务统一多选，并在业务行内维护是否协调与协调时间。
- [x] 为割接编辑视图补充裸纤业务候选列表上下文，包含业务 ID、租户组和业务名称。
- [x] 调整割接表单与编辑模板，隐藏原 JSON 字段，保留 JSON 存储但用多选弹窗和行内控件维护数据。
- [x] 运行割接管理相关测试和编辑页内联脚本语法检查。

## 2026-05-15 裸纤业务故障统计口径调整
- [x] 增加源码级回归测试，锁定裸纤业务故障统计按影响业务中的裸纤业务且业务影响为业务中断统计。
- [x] 调整业务统计后端查询，裸纤业务不再限定物理故障类型为光缆中断，改为统计所有类型物理故障。
- [x] 运行定向测试和 Python 编译检查验证口径调整。

## 2026-05-12 故障月历四类故障配色
- [x] 增加源码级回归测试，锁定故障月历仅显示光缆中断、空调故障、供电故障、设备故障四类圆点和图例。
- [x] 调整月历小组件后端过滤口径与分类颜色，按图中配色输出圆点。
- [x] 运行定向测试验证月历小组件变更。

## 2026-05-06 表单错误字段自动定位
- [x] 增加回归测试，锁定故障编辑页加载后滚动并聚焦第一个错误字段。
- [x] 调整故障编辑页脚本，查找 NetBox 渲染出的错误提示或错误控件并跳转。
- [x] 运行定向测试验证变更。

## 2026-05-06 区分复核角色检查业务故障类型
- [x] 调整回归测试，锁定线路主管复核检查未恢复裸纤业务故障、运维主管复核检查未恢复电路业务故障。
- [x] 调整故障编辑表单校验，按复核角色分别过滤业务故障类型。
- [x] 调整编辑页即时提示，两个复核勾选框分别展示对应业务类型提示。
- [x] 运行定向测试验证变更。

## 2026-05-06 线路主管复核检查未恢复业务故障
- [x] 增加回归测试，锁定线路主管复核设置为真时会检查下挂业务故障恢复时间。
- [x] 在故障编辑表单中统计下挂未恢复业务故障，并在保存时阻止复核通过。
- [x] 在编辑页勾选线路主管复核时给出即时提示，引导用户检查未恢复业务故障。
- [x] 运行定向测试验证变更。

## 2026-04-30 业务故障页按业务模型字段分组
- [x] 增加源码级回归测试，锁定业务统计 API 为裸纤业务返回租户组分组、为电路业务返回业务门类分组。
- [x] 调整业务统计后端响应，为每个业务卡片补充 `group_label`，空值统一归入“未分组”。
- [x] 调整业务故障卡片前端渲染，按 `group_label` 输出分组标题和组内卡片网格，并保持点击下钻与月历图表正常。
- [x] 补充分组标题与嵌套卡片网格样式，提升静态资源版本并运行定向验证。

## 2026-04-29 整理业务中断卡片实施文档
### 实施步骤
- [x] 梳理图片中的业务中断卡片结构、现有业务统计 API 与前端渲染入口。
- [x] 编写实施文档，明确统计口径、响应结构、前端布局、测试和实施步骤。
- [x] 登记计划项，便于后续按文档拆分开发任务。

## 2026-04-30 业务故障卡片增加年度累计区
- [x] 增加源码级回归测试，锁定业务故障卡片顶部年度累计标题和三格指标布局。
- [x] 调整业务故障卡片渲染，在卡片最上方加入年度累计（2026年）和 SLA/中断时长/中断起数三项。
- [x] 补充样式并运行定向测试与 JS 语法检查。

## 2026-04-30 业务故障卡片增加本期间分类明细
- [x] 增加源码级回归测试，锁定业务统计 API 返回分类次数与分类时长数组。
- [x] 调整业务故障卡片年度累计下方的本期间区，展示故障总数和故障时长两项分类分解。
- [x] 去除业务卡片“其他”分类兜底展示，改为按故障分类全量列示。
- [x] 运行业务卡片定向测试、后端源码测试和 JS 语法检查。

## 2026-04-30 跳过光缆中断旧结构失效测试
- [x] 将 `tests.test_statistics_cable_break_overview` 中 9 个失败和 5 个错误的旧结构断言测试标记为 skip。
- [x] 重新运行完整 `tests.test_statistics_cable_break_overview`，确认剩余测试通过且 14 个测试被跳过。

## 2026-04-30 业务故障本期间表格化美化
- [x] 增加源码级回归测试，锁定本期间改为浅分割线表格并同一行展示分类、故障总数和故障时长。
- [x] 调整业务故障卡片本期间渲染与样式，提高横向空间利用率并减少纵向占用。
- [x] 运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障本期间改为矩阵表格
- [x] 增加源码级回归测试，锁定本期间使用左侧指标总览、右侧分类矩阵的表格结构。
- [x] 调整本期间渲染为上下两块矩阵：故障总数和故障时长。
- [x] 调整浅分割线样式并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障卡片按视觉稿重绘
- [x] 增加源码级回归测试，锁定年度标题状态栏、本期间大汇总和分类表格结构。
- [x] 调整业务故障卡片 JS 结构，增加标题图标、状态胶囊、底部指标列表和服务名色条。
- [x] 调整业务故障卡片 CSS，使宽卡片、浅表格分割线和指标层级接近视觉稿。
- [x] 运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障卡片紧凑化
- [x] 增加源码级回归测试，锁定分类分项仅显示数字且卡片整体缩小。
- [x] 调整业务故障卡片分项渲染，去掉分类表格右侧值的单位。
- [x] 下调业务故障卡片宽度、高度、留白和字体，使其贴合系统界面密度。
- [x] 运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障卡片去除底部指标
- [x] 增加源码级回归测试，锁定业务故障卡片宽度再缩小 30% 且不渲染底部指标列表。
- [x] 移除业务故障卡片平均时长、长时故障、重复故障、千公里故障率和 SLA 指标渲染。
- [x] 调整业务故障卡片宽度、高度和底部间距。
- [x] 运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障卡片字体对齐物理故障卡片
- [x] 增加源码级回归测试，锁定业务卡片数字、单位、标签和分类文字使用接近物理故障卡片的字号粗细。
- [x] 调整业务卡片年度区、本期间汇总区、分类表格和标题栏字体层级。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障分项指标取消粗体
- [x] 增加源码级回归测试，锁定本期间右侧分类名称和值均不使用粗体。
- [x] 调整业务故障卡片分类分项字体粗细。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障年度累计图标调整
- [x] 增加源码级回归测试，锁定年度累计使用无边框年度统计语义图标。
- [x] 替换业务故障卡片年度累计图标并去除图标边框线。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障本期间改为摘要行
- [x] 增加源码级回归测试，锁定本期间不再使用表格矩阵，改为两条摘要指标行。
- [x] 调整业务故障卡片本期间渲染，展示指标名、总值和非零分类摘要。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障卡片增加运行月历
- [x] 增加源码级回归测试，锁定业务统计 API 返回本年 1-12 月月度故障数与时长。
- [x] 在业务故障卡片下方新增运行月历栏，使用无边界 ECharts 展示月度柱线组合图。
- [x] 调整前端图表初始化、销毁和 resize 逻辑，确保服务卡片重绘后图表正常。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障运行月历标题与月份标签调整
- [x] 增加源码级回归测试，锁定运行月历使用与本期间一致的标题图标字体，并为月份标签预留底部空间。
- [x] 调整运行月历标题样式和 ECharts 下方 grid/axisLabel，避免 1月/2月等文字裁剪。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障运行月历改为单图双轴
- [x] 增加源码级回归测试，锁定运行月历使用单个 ECharts grid 和双 Y 轴，折线通过轴范围显示在柱状图上方。
- [x] 调整 ECharts 配置，移除上下双 grid，减少柱线间隔并保持月份标签完整。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障运行月历时长曲线日粒度化
- [x] 增加源码级回归测试，锁定业务统计 API 返回本年每日故障时长序列。
- [x] 调整运行月历图表，X 轴改为全年日粒度，月故障数柱放在月中位置，故障时长折线使用每日数据。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障运行月历时长恢复月粒度
- [x] 增加源码级回归测试，锁定运行月历故障时长折线使用月度时长数据。
- [x] 调整运行月历图表，X 轴恢复 1-12 月，柱状图和折线图均使用月度序列。
- [x] 清理不再使用的每日时长返回字段，提升静态资源版本并运行验证。

## 2026-04-30 业务故障运行月历增加横坐标线
- [x] 增加源码级回归测试，锁定运行月历 X 轴显示轻量横坐标线。
- [x] 调整 ECharts xAxis axisLine，仅显示横坐标基线并保持无边界嵌入。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障卡片增加中断日历
- [x] 增加源码级回归测试，锁定业务统计 API 返回上上月、上月、本月每日中断次数。
- [x] 在业务故障卡片底部新增无标题中断日历，按日方块展示并用红色深浅表示故障数。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障中断日历按星期排布
- [x] 增加源码级回归测试，锁定每月返回首日星期并在前端插入前置空白格。
- [x] 调整中断日历渲染，使日期方块按真实星期列对齐。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。

## 2026-04-30 业务故障中断日历统一颜色标尺
- [x] 增加源码级回归测试，锁定中断日历颜色分档使用所有业务的三个月每日最大值。
- [x] 调整前端渲染，将全局最大值传入每张业务卡片的中断日历。
- [x] 提升静态资源版本并运行定向测试、完整统计页源码测试和 JS 语法检查。
...
### 实施步骤
- [x] 扩展 `LayerToggleControl`，支持通过配置隐藏故障分布图专属的视图模式、时间范围和故障类型筛选，仅保留“全部站点与路径”图层开关。
...
## 2026-04-28 故障统计平均历时行增加历时指标卡片
### 实施步骤
- [x] 增加回归测试，锁定“历时指标”卡片位置、四项指标、超时率 `>=4` 小时口径和平均历时行布局。
- [x] 后端在光缆中断概览 `avg_metrics` 中补充 MTTR、P50、P90、超时率及上期对比数据。
- [x] 前端在平均历时行新增四指标卡片，并沿用现有红绿箭头趋势渲染。
- [x] 调整平均历时行 CSS，让重复中断卡片缩小且整行对齐。
- [x] 运行定向测试、统计页相关测试和 JS/Python 语法检查。

## 2026-04-28 历时指标卡片去除重复 MTTR
### 实施步骤
- [x] 更新回归测试，要求“历时指标”只包含 P50、P90、超时率，避免与全口径平均重复。
- [x] 移除后端 `mttr_avg` 字段和前端 MTTR 指标渲染。
- [x] 调整“历时指标”卡片为三指标布局，并保持平均历时行对齐。
- [x] 运行定向测试、统计页相关测试和 JS/Python 语法检查。

## 2026-04-28 指标说明补充历时指标
### 实施步骤
- [x] 更新回归测试，锁定指标说明包含 P50、P90 和超时率 `>=4` 小时口径。
- [x] 在指标说明模态窗的光缆中断说明中补充三项历时指标定义。
- [x] 运行指标说明定向测试。

## 2026-04-28 修正历时指标下钻语义
### 实施步骤
- [x] 更新回归测试，要求 P50 点击过滤 `duration_max`，P90 点击过滤 `duration_min`。
- [x] 前端将 P50/P90 指标卡下钻字段改为对应历时阈值，并扩展明细过滤逻辑。
- [x] 更新过滤条件显示文案，明确 `<=P50` 与 `>=P90`。
- [x] 运行定向测试和统计页相关测试。
...
## 2026-04-28 盒须图默认启用对数刻度
### 实施步骤
- [x] 更新源码级回归测试，要求 `physical-boxplot-log-scale` 默认带 `checked`。
- [x] 调整统计页模板，让“对数刻度”开关默认选中。
- [x] 运行统计页定向测试、统计页相关测试和 JS 语法检查。

## 2026-04-28 盒须图 tooltip 显示离散点
### 实施步骤
- [x] 补充源码级回归测试，锁定 tooltip 会读取 scatter 离散点原始值并追加“超出上须”信息。
- [x] 更新 `statistics_dashboard.js` 的盒须图 tooltip formatter，合并当天离散点并显示历时。
- [x] 运行统计页定向测试、统计页相关测试和 JS 语法检查。

## 2026-04-28 调整盒须图对数坐标最小刻度
### 实施步骤
- [x] 更新源码级回归测试，要求对数模式下最小刻度和零值钳制下界为 `0.01`。
- [x] 将 `statistics_dashboard.js` 中 log 轴最小绘图值、Y 轴下界和刻度文本改为 `0.01`。
- [x] 运行统计页定向测试、统计页相关测试和 JS 语法检查。

## 2026-04-27 修正盒须图对数刻度零值显示
### 实施步骤
- [x] 补充源码级回归测试，锁定对数模式下盒须图/异常点绘图值会避开 `<=0`，并显示 `0.1` 下界刻度。
- [x] 更新 `statistics_dashboard.js`，对 log 轴绘图数据做最小正值钳制，同时保留 tooltip 原始值。
- [x] 运行统计页定向测试和 JS 语法检查。

## 2026-04-27 增加盒须图对数刻度开关
### 实施步骤
- [x] 补充源码级回归测试，锁定盒须图控件区包含“对数刻度”开关、前端绑定 change 事件并按开关切换 Y 轴类型。
- [x] 运行定向测试，确认当前缺少该开关和轴类型切换逻辑。
- [x] 更新统计页模板和 `statistics_dashboard.js`，增加对数刻度开关并在渲染盒须图时设置 `yAxis.type`。
- [x] 刷新统计页 JS 版本号并运行定向测试、JS 语法检查。

## 2026-04-27 调整故障统计盒须图上须和异常点
### 实施步骤
- [x] 补充源码级回归测试，锁定盒须图上须使用 `Q3 + 1.5 * IQR`，并为超过上须的样本返回独立异常点数据。
- [x] 运行定向测试，确认当前实现仍把最大样本值作为上须且缺少异常点 series。
- [x] 调整后端盒须统计与前端 ECharts 配置，使用 scatter 小圆点展示超范围样本。
- [x] 运行定向 unittest、JS 语法检查和 Python 语法检查。
# 2026-04-28 业务故障页面增加故障明细下钻列表

## 实施步骤
- [x] 补充源码级回归测试，锁定裸纤业务故障和电路业务故障 Tab 均包含独立故障明细表。
...
## 2026-05-01 为路径组增加复制其他路径组成员功能
- [x] 增加源码级回归测试，锁定复制表单包含源路径组与复制模式，且不能选择当前路径组。
- [x] 增加路径组复制视图，支持保留现有成员合并复制或覆盖现有成员后复制。
- [x] 增加路径组复制 URL 与详情页入口按钮。
- [x] 运行定向测试和 Python 语法检查。

## 2026-05-01 修复路径组地图初始化视野
- [x] 增加源码级回归测试，锁定路径组地图初始化时按路径与站点的合并边界缩放。
- [x] 调整 `location_mode.js`，为 `pathgroup` 模式统一计算高亮路径和高亮站点边界。
- [x] 运行定向测试和 JavaScript 语法检查。
## 2026-05-01 故障分布图路径组叠加选择
### 实施步骤
- [x] 新增路径组地图叠加数据 API，一次返回路径组名称、路径 GeoJSON 和站点角色点位，复用路径组查看地图所需的数据结构。
- [x] 在故障分布图配置中注入路径组叠加 API URL，并在 `LayerToggleControl` 的“全部站点与路径”下方渲染所有路径组复选项。
- [x] 选中路径组时按路径组查看地图的配图方式绘制金色路径高亮、角色色站点圆点/光晕/标签；取消选中时隐藏对应叠加层。
- [x] 保持路径组查看地图的拓扑专用菜单行为不受影响。
- [x] 增加源码级回归测试并运行定向测试。

## 2026-05-01 故障分布图路径组显示策略解耦
### 实施步骤
- [x] 增加源代码级回归测试，锁定关闭“全部站点与路径”时已选择的路径组覆盖层仍保持显示。
- [x] 调整 `LayerToggleControl`，让全量站点与路径开关只控制底图图层，不再控制路径组覆盖层。
- [x] 保持路径组复选框独立控制覆盖层显示、隐藏和自动缩放。
- [x] 运行路径组覆盖层定向测试和 JavaScript 语法检查。
## 2026-05-01 故障月历显示上月尾部日期
- [x] 增加源码级回归测试，锁定月历 1 号之前的上月日期也查询并填充故障点。
- [x] 调整月历后端网格构造，使用真实日期作为故障点索引并生成上月尾部日期单元格。
- [x] 调整月历模板，为非本月日期使用灰色日期文字。
- [x] 运行定向测试确认月历小组件更新。
## 2026-05-02 故障月历标记休班日期
...
## 2026-05-07 业务故障卡片运行月历逐月指标显示
- [x] 调整回归测试，锁定每个月的柱状图/折线图数据点直接显示对应指标。
- [x] 移除运行月历总计 `graphic`，改为柱状图显示每月故障数、折线点显示每月故障时长。
- [x] 运行定向测试、完整统计页源码测试和 JS 语法检查验证变更。
## 2026-05-07 业务故障月历统计口径修正
- [x] 增加源码级回归测试，锁定本期间分类统计保留原口径，运行月历、运行日历只统计光缆中断且非挂起故障。
- [x] 调整业务统计 Impact 查询过滤条件，仅对年度月历与近三个月日历统一关联故障类型和状态口径。
- [x] 运行定向测试、完整统计页源码测试和 Python 语法检查验证变更。
## 2026-05-07 业务运行日历颜色分级调整
- [x] 增加源码级回归测试，锁定 1/2/3/4+ 次运行日历颜色。
- [x] 调整运行日历四档颜色为浅蓝、深蓝、浅红、深红。
- [x] 运行定向测试和完整统计页源码测试验证变更。
## 2026-05-07 业务运行日历绝对次数分级
- [x] 增加源码级回归测试，锁定日历颜色按绝对次数 1/2/3/4+ 分级。
- [x] 调整运行日历等级函数，去掉按最大值比例映射导致 2 次显示红色的问题。
- [x] 运行定向测试和完整统计页源码测试验证变更。
## 2026-05-07 裸纤业务零故障卡片补全
- [x] 增加回归测试，锁定业务故障统计接口返回所有裸纤业务并标记本期是否发生故障。
...
- [x] 运行定向测试验证新增字段覆盖完整。
## 2026-05-08 业务影响字段颜色旗标显示
- [x] 增加源代码级回归测试，锁定业务影响选项颜色为业务中断红色、业务未中断蓝色。
- [x] 调整故障影响业务详情页，使用 NetBox badge 旗标显示业务影响。
- [x] 确认列表页继续使用 `ChoiceFieldColumn` 颜色旗标展示业务影响。
- [x] 运行定向测试和语法检查验证变更。

## 2026-05-08 故障详情影响业务操作列保持最后
- [x] 增加源代码级回归测试，锁定故障详情页影响业务表格中 `actions` 永远为最后一列。
- [x] 调整 `OtnFaultImpactSummaryTable` 列顺序，将 `business_impact` 放在操作列前。
- [x] 将“操作列永远最后”的规则写入项目约束。
- [x] 运行定向测试和语法检查验证变更。

## 2026-05-08 故障影响业务增加业务影响字段
- [x] 增加源代码级回归测试，锁定 `OtnFaultImpact.business_impact` 字段、选项、表单顺序、详情页顺序、筛选与 API 暴露。
...
- [x] 运行定向测试和 JS 语法检查验证变更。
## CircuitService Excel 扩展字段 JSON

- [x] 为 `CircuitService` 新增统一 JSON 扩展字段，保存 Excel 中暂不结构化的缺失表头。
- [x] 在编辑表单中按字段拆分录入扩展信息，保存时统一写入 JSON。
- [x] 在详情页按字段拆分展示扩展信息。
- [x] 调整 Excel 读取和导入逻辑，将缺失表头以文本写入 JSON。
- [x] 增加源码级回归测试，覆盖字段定义、表单字段、详情展示和 Excel 读取。

## CircuitService 扩展信息导入脚本

- [x] 新增 NetBox 自定义脚本，从 Excel 读取扩展字段并更新现有电路业务。
- [x] 脚本按“专线名称 + 电路编号”定位 `CircuitService`，不创建新业务。
- [x] Excel 有值的扩展字段覆盖旧值，Excel 空值不清空系统已有值。
- [x] 增加源码级回归测试，覆盖提交写入、模拟运行和未匹配记录日志。

## 2026-05-09 涉及88系统故障对部周报CSV脚本
- [x] 梳理现有 `OtnFault` 字段、标签过滤方式和自定义脚本目录，确认只在插件目录内新增脚本与服务模块。
- [x] 新增源代码级回归测试，锁定按输入日期计算周一 0 点至下周一 0 点、筛选涉及88系统标签、同日同分类口径/A端/Z端/一级原因合并、光缆类优先合并到第一条中断类故障、光缆整改填“否，整改”等规则。
- [x] 新增可单测的对部周报统计服务，输出 CSV 行数据并提供 Excel 友好的 UTF-8 BOM CSV 文本。
- [x] 新增 NetBox 自定义脚本，提供日期参数，查询当周带“涉及88系统”标签的故障，调用统计服务并返回 CSV 内容。
- [x] 运行定向测试验证合并规则和脚本入口。
- [x] 增加脚本加载回归测试，覆盖 NetBox 脚本加载环境无法直接导入插件服务子包时模块仍可加载。
- [x] 将统计服务改为运行时延迟加载，并用脚本相邻路径兜底，避免自定义脚本列表加载阶段失败。
- [x] 增加运行时导入回归测试，覆盖已安装 `netbox_otnfaults.services` 包存在但缺少本次新增服务模块时仍可输出周报。
- [x] 为自定义脚本加入内置统计兜底逻辑，避免只部署脚本文件、未同步安装服务模块时运行失败。
- [x] 为脚本返回结果设置 CSV 下载元数据：`text/csv; charset=utf-8`、`.csv` 扩展名和按统计日期生成的文件名。
- [x] 按部署要求移除 `services/ministry_weekly_fault_report.py`，将周界、合并、CSV 渲染逻辑全部保留在自定义脚本文件内。
- [x] 调整测试为只加载 `scripts/ministry_weekly_fault_report.py`，并锁定脚本源码不再导入或查找 service 模块。
## 态势大屏图层顺序固定
- [x] 增加源码级回归测试，锁定大屏地图自下而上为省份、基础路径、站点、热力、故障路径、故障点。
- [x] 先运行新增测试，确认当前仅重排基础拓扑且热力图插入在站点之前。
- [x] 在 `dashboard/map_engine.js` 中增加统一图层栈重排函数，并在省份、路径、站点、热力、故障点渲染后调用。
- [x] 运行新增测试、现有大屏地图测试与 JS 语法检查。

## 态势大屏故障聚焦站点名称
- [x] 增加源码级回归测试，锁定活跃故障数据包含 A/Z 站点 ID，故障聚焦时地图会同步高亮这些站点名称。
- [x] 先运行新增测试，确认当前聚焦流程不会更新站点名称显示过滤器。
- [x] 在 `dashboard_views.py` 补充故障关联站点 ID，并在 `dashboard/map_engine.js` 增加聚焦站点标签层与过滤函数。
- [x] 在 `dashboard_app.js` 的故障进入/离开回调中调用站点名称聚焦与清理。
- [x] 运行新增测试、现有大屏地图测试与 JS 语法检查。

## 态势大屏故障聚焦普通站点名称
- [x] 增加源码级回归测试，锁定故障聚焦 zoom 由站点名称 minzoom 门槛计算得出，而不是固定魔法数字。
- [x] 先运行新增测试，确认当前故障聚焦镜头没有和普通站点名称门槛建立关系。
- [x] 在 `map_engine.js` 暴露站点名称显示门槛，在 `directing_engine.js` 用门槛加余量计算聚焦 zoom。
- [x] 运行新增测试、现有大屏地图测试与 JS 语法检查。

## 态势大屏故障聚焦普通站点标签碰撞
- [x] 增加源码级回归测试，锁定故障聚焦时普通站点标签临时允许重叠并忽略碰撞避让。
- [x] 先运行新增测试，确认当前只提高 zoom，普通 `sites-label` 仍会被符号碰撞隐藏。
- [x] 在 `map_engine.js` 的故障聚焦/离开逻辑中切换普通站点标签布局属性，聚焦结束后恢复干净视图。
- [x] 运行新增测试、现有大屏地图测试与 JS 语法检查。

## 态势大屏故障聚焦普通站点点位增强
- [x] 增加源码级回归测试，锁定故障聚焦时普通站点点位临时放大、加亮描边和光晕。
- [x] 先运行新增测试，确认当前只调整标签，站点点位仍是 3px 低对比样式。
- [x] 在 `map_engine.js` 的故障聚焦/离开逻辑中切换普通站点点位 paint 属性，聚焦结束后恢复。
- [x] 运行新增测试、现有大屏地图测试与 JS 语法检查。

## 态势大屏故障涉站点名称去重
- [x] 增加源码级回归测试，锁定故障聚焦时普通站点名称层排除 A/Z 涉站点，避免与聚焦站点名称重复。
- [x] 先运行新增测试，确认当前普通 `sites-label` 与 `sites-focus-label` 会同时显示同一站点名称。
- [x] 在 `map_engine.js` 中按 `focusedSiteIds` 设置普通站点名称过滤器，并在清理聚焦时恢复过滤器。
- [x] 运行新增测试、现有大屏地图测试与 JS 语法检查。

## 态势大屏站点与基础路径绿色系
- [x] 增加源码级回归测试，锁定站点图层、基础拓扑路径使用绿色系，故障关联路径保留红色系。
- [x] 先运行新增测试，确认当前站点和路径仍混用青色、蓝色或红色。
- [x] 调整 `dashboard/map_engine.js` 中站点与基础路径相关颜色配置，保留活跃故障状态色和故障关联路径红色不变。
- [x] 运行新增测试、现有大屏地图测试与 JS 语法检查。

## 态势大屏热力图视觉降温
- [x] 增加源码级回归测试，锁定大屏热力图低强度、小半径、后移红色阈值和较低透明度。
- [x] 先运行新增测试，确认当前热力图参数仍会过早进入大面积红色。
- [x] 调整 `dashboard/map_engine.js` 的 `closed-heatmap-layer` paint 配置，保留本年已关闭故障口径，仅降低视觉饱和度。
- [x] 运行新增测试与 JS 语法检查，确认样式参数和脚本语法可用。
## 2026-05-09 态势大屏故障聚焦站点显示稳定性
- [x] 增加源码级回归测试，锁定故障聚焦站点点层进入统一图层栈、聚焦站点标签位于热力图之上且故障路径之下。
- [x] 统一站点 GeoJSON 与故障聚焦 ID 为字符串，避免 MapLibre filter 因数字/字符串类型不一致导致站点闪现后消失。
- [x] 将聚焦站点点层与标签层 filter 统一改为 expression 语法，并在聚焦/离开时重新应用图层顺序。
- [x] 聚焦时仅放大故障涉及站点点位；普通站点点位保持原尺寸，仅普通站点标签放开碰撞。
## 2026-05-11 裸纤业务故障卡片范围拨块
- [x] 增加源码级回归测试，锁定裸纤业务故障 tab 专属的业务范围拨块、默认只显示本期有故障业务、切换后前端重绘卡片。
- [x] 调整故障统计页 toolbar 模板，在现有筛选控件左侧加入仅裸纤业务 tab 显示的双选项拨块。
- [x] 调整统计页前端脚本，缓存裸纤业务服务列表，按拨块状态过滤 `service-cards-container`，并在 tab 切换时同步显示状态。
- [x] 调整统计页样式，保证拨块与右上角 toolbar 对齐，移动端可换行且默认隐藏。
- [x] 运行定向源码测试与 JS 语法检查。
## 2026-05-11 故障分布图路径组加载优化
- [x] 增加源代码级回归测试，锁定路径组菜单先加载轻量元数据，单组选中后再按需加载详情。
- [x] 调整路径组地图 API：列表接口不返回路径几何，新增单组详情接口按需返回路径组 GeoJSON。
- [x] 调整 `LayerToggleControl.js`：固定合并 source/layer，避免每个路径组创建一套图层；选中/取消时刷新合并数据。
- [x] 确认路径组覆盖层不依赖 PMTiles，PMTiles 导出脚本不写入路径组属性。
- [x] 运行定向测试和 Python/JavaScript 语法检查。
## 2026-05-11 修复线路设计器地图配置初始化
- [x] 增加回归测试，锁定统一地图核心在配置对象缺失或仅存在兼容配置时的行为。
- [x] 调整统一地图核心初始化，避免读取未定义配置的 `apiKey` 并兼容旧配置键。
- [x] 运行定向测试验证线路设计器入口初始化逻辑。
## 2026-05-11 修复线路设计器配置脚本语法错误
- [x] 增加回归测试，锁定线路设计器传入地图偏好上下文并且模板对缺省偏好输出合法 JSON。
- [x] 补齐线路设计器视图上下文，同时在统一地图模板中为 `mapStylePreferences` 增加兜底。
- [x] 运行定向测试和相关 JS 语法检查。
## 2026-05-12 供电维护方式字段命名与列表展示
- [x] 增加源码级回归测试，锁定 `power_maintenance_mode` 展示名改为“供电维护方式”，并出现在主故障列表字段中。
- [x] 调整模型、表单、详情页与列表列配置，保留原通用 `maintenance_mode` 的“维护方式”文案不变。
- [x] 校验 `OtnFaultTable.Meta.fields` 中新增列位于 `actions` 之前，且操作列仍保持最后一列。
- [x] 运行定向测试验证变更。
## 2026-05-15 SLA 精度统一为两位向下截断
- [x] 增加源码级回归测试，锁定业务统计接口、业务详情页和前端 SLA 展示均使用两位小数向下截断。
- [x] 增加后端 SLA 截断 helper，并替换月度、年度、本期和详情页本月 SLA 输出。
- [x] 增加前端 SLA 专用格式化函数，避免通用一位小数格式覆盖 SLA 精度。
- [x] 运行定向测试和 Python 编译检查验证变更。
## 2026-05-18 统一故障地图定位策略
- [x] 增加回归测试，锁定故障 GPS、单站点 A 端、多站点单 Z 路径中点、多 Z 回退 A 端、A 端无坐标不返回坐标的统一策略。
- [x] 新增共享故障坐标解析服务，保留 `coords_from_site` / `coords_source` 非精确坐标标记。
- [x] 替换故障分布图、光缆中断统计地图、大屏故障点和详情定位地图入口中的分散定位逻辑。
- [x] 运行定向测试和 Python 编译检查验证变更。
# 割接管理第一阶段开发计划（2026-05-18）

## 目标
- 基于 `PRD/割接管理.md` 新增独立的割接管理对象，完成 NetBox 插件内的基础 CRUD、筛选、导入、批量编辑、REST API 和导航入口。
- 第一阶段不实现地图点选、GraphQL、统计大屏、附件专项 UI；仅保留经纬度、JSON 结构字段和 NetBox 通用附件/日志能力的模型基础。

## 步骤
1. 先写 `tests/test_cutover_management_scaffold.py`，用源码断言锁定第一阶段应交付的模型、表单、表格、视图、URL、API、导航和模板结构。
2. 在 `netbox_otnfaults/models.py` 新增割接状态、超时状态、客户审核、割接结果、管理单位等 ChoiceSet，以及 `CutoverTask` 模型。
3. 新增迁移文件，创建 `CutoverTask` 表、站点多对多表和必要索引/约束。
4. 在 `filtersets.py`、`forms.py`、`tables.py` 中补齐筛选、编辑、导入、批量编辑和列表表格。
5. 在 `views.py`、`urls.py`、`navigation.py` 中接入列表、详情、编辑、删除、批量导入/编辑/删除和菜单入口。
6. 在 `api/serializers.py`、`api/views.py`、`api/urls.py` 中接入 REST API。
7. 新增详情页与编辑页模板，使用 NetBox/Bootstrap 原生布局。
8. 运行新增源码测试和可用的 Python 编译检查。

## 风险与取舍
- `planned_cutover_times` 与 `related_customers` 第一阶段采用 `JSONField`，避免过早拆分 InlineModel。
- `handling_unit` 与 `contract` 复用当前项目已使用的 `netbox_contract.models.ServiceProvider` 和 `Contract`。
- 地图选点只保留经纬度字段，交互式地图放入后续阶段。

## 割接编号调整（2026-05-18）
- 将 `cutover_no` 调整为系统自动生成，格式为 `CYYYYNNNN`，例如 `C20260001`。
- 年份取当前本地日期年份，流水号按年度递增，使用事务锁与唯一约束防止并发重复。
- 表单不允许录入或修改割接编号；已有对象只显示只读编号，新建对象显示“保存后自动生成”。
## 2026-05-19 补齐割接详情页字段展示
- [x] 增加源码级回归测试，锁定割接详情页展示资源信息、客户审核明细、线路主管和整改字段。
- [x] 在割接详情页按现有五五布局补充客户审核、资源信息、组织联系人和整改信息展示。
- [x] 运行定向测试、编译检查和 diff 空白检查。

## 2026-05-19 补齐故障详情页时间字段展示
- [ ] 增加源码级回归测试，锁定故障详情页展示 `OtnFault` 时间字段和历时字段。
- [ ] 在故障详情页时间轴下方补充标准表格卡片，展示故障起始、派发、出发、到场、修复、恢复、封包、故障历时和处理历时。
- [ ] 运行定向测试、编译检查和 diff 空白检查。

## 2026-05-19 删除割接任务实施时间线业务中断字段
- [x] 在回归测试中锁定 `CutoverTask` 不再包含业务中断时间、业务恢复时间、实际中断时长。
- [x] 从模型、迁移、表单、API、割接详情页和编辑页移除这三个割接任务级字段。
- [x] 更新割接管理文档和割接影响业务实现计划，避免继续引用父割接的业务中断/恢复时间。
- [x] 运行定向测试、编译检查和 diff 空白检查。

## 2026-05-20 割接详情页生成新时间模态确认
- [x] 参考 netbox-contract 用印事由弹窗样式，把割接详情页“生成新的割接时间”从浏览器确认框改为 Bootstrap 模态框。
- [x] 保留原有时间输入和提交接口，确认后再提交，并在弹窗中说明会重置关联用户确认状态和协调时间。
- [x] 运行割接详情页定向测试、脚本语法检查和 Python 编译检查。

## 2026-05-21 删除割接客户审核结果字段
- [x] 从 `CutoverTask` 模型、迁移和 API 序列化中移除 `customer_approval_result`。
- [x] 从割接表单、导入、批量编辑和详情页展示中移除客户审核结果，保留客户审核明细字段。
- [x] 更新割接管理源码级测试并运行定向验证、编译检查和 diff 检查。

## 2026-05-21 删除割接维护单位字段
- [x] 从 `CutoverTask` 模型、迁移、API 序列化中移除 `maintenance_unit`。
- [x] 从割接表单、导入表单、编辑页和详情页组织联系人卡片中移除维护单位。
- [x] 更新源码级测试并运行定向验证、编译检查和 diff 检查。

## 2026-05-21 调整割接编辑页字段分组顺序
- [x] 按详情页顺序调整 `CutoverTaskForm.fieldsets`：割接信息、割接位置、资源信息、组织联系人、计划割接时间、关联用户、实施时间线、考核与闭环、整改信息。
- [x] 同步调整 `cutovertask_edit.html` 卡片标题和字段渲染顺序，不改变字段行为。
- [x] 更新源码级测试并运行定向验证、编译检查和 diff 检查。
## 2026-05-21 故障分布图图层显示入口独立
- [x] 增加源代码级回归测试，锁定故障分布图左侧筛选菜单不再包含其他图层，右侧新增图层显示按钮并放在地图样式设置之上。
- [x] 调整 `LayerToggleControl.js`，支持自定义按钮图标/菜单定位，并让仅图层模式复用现有“其他图层 + 路径组”渲染逻辑。
- [x] 调整 `fault_mode.js`，拆分筛选控制与图层显示控制，分别维护筛选状态和基础/路径组图层显示状态。
- [x] 运行定向测试与 JavaScript 语法检查验证变更。

## 2026-05-22 割接日历小组件
- [x] 增加源码级回归测试，锁定新增割接月历 Widget 使用计划割接时间、状态颜色、影响业务数量和列表筛选链接。
- [x] 在 Dashboard 中新增割接月历 Widget，按月历可见范围查询割接并按日期聚合紧凑摘要。
- [x] 新增割接月历模板，日期格最多显示两条摘要和更多数量提示，完整信息通过标题和跳转查看。
- [x] 运行定向测试和 Python 编译检查验证变更。
## 2026-05-22 割接日历摘要三行显示
- [x] 增加源码级回归测试，锁定割接日历每条摘要按“时间+省份 / 具体地址 / 影响业务”三行显示。
- [x] 调整割接日历后端 payload，补充省份显示字段，并将单日可见割接收紧为 1 条。
- [x] 调整割接日历模板与样式，适配三行摘要和小组件高度。
- [x] 运行定向测试和 Python 编译检查验证变更。
## 2026-05-22 修复割接维护单位遗留列
- [x] 增加源码级回归测试，锁定新增幂等迁移会清理 `CutoverTask.maintenance_unit` 遗留数据库列。
- [x] 新增迁移，若 `netbox_otnfaults_cutovertask.maintenance_unit` 仍存在则直接删除。
- [x] 运行割接管理定向测试和 Python 编译检查验证变更。
## 2026-05-22 隐藏割接客户审核明细 JSON 字段
- [x] 增加源码级回归测试，锁定割接编辑页不再可见渲染 `customer_approval_detail` JSON 文本框。
- [x] 调整割接编辑页模板，将 `customer_approval_detail` 改为隐藏字段提交，避免覆盖已有审核明细。
- [x] 运行定向测试和 Python 编译检查验证变更。

## 2026-05-22 修复一张图故障模式脚本语法错误
- [x] 增加源码级回归测试，使用 `node --check` 锁定 `fault_mode.js` 不再出现孤立 `else` 语法错误。
- [x] 删除割接发光动画方法后残留的重复 `else` 分支，保留 `highlightPath()` 内已有的静态高亮兜底逻辑。
- [x] 运行定向测试和 JavaScript 语法检查验证页面脚本可解析。


## 2026-05-22 修复一张图割接图层入口与显示状态
- [x] 增加源码级回归测试，锁定割接开关位于右侧图层显示控件，且割接可见性读取同一个控件实例。
- [x] 增加资源缓存版本测试，确保 `LayerToggleControl.js` 的割接菜单改动能被浏览器重新加载。
- [x] 调整 `fault_mode.js` 的割接图层显示状态读取来源，并更新 `map_modes.py` 中控件脚本版本号。
- [x] 运行定向测试、JavaScript 语法检查和 Python 编译检查。

## 2026-06-01 割接生成故障实施文档
- [x] 明确采用“预填确认”流程，不从割接详情页静默创建故障。
- [x] 明确新增 `OtnFault.source_cutover_task` 结构化关联，用于反查来源割接和防重复生成。
- [x] 明确 `rectification_*` 字段不从割接映射到故障。
- [x] 编写完整实施方案与可执行开发计划，覆盖模型、表单、视图、模板、URL、测试和迁移验收。

## 2026-06-01 故障统计分公司页面
- [x] 增加源码级回归测试，锁定分公司 Tab 位于电路业务之后、复用顶部时间筛选、后端返回六省分公司统计 payload。
- [x] 在故障统计后端增加六省分公司聚合：浙江、山东、内蒙、陕西、四川、江西；总体情况和光缆中断情况按六省过滤，图表数据包含原始值与百公里归一化值。
- [x] 在统计模板新增分公司 Tab，复用总体情况、光缆中断情况的卡片结构，并新增三块图表区域：故障数/历时柱状图、修复时长箱线图/有效时长柱状图、年初至今周趋势折线图。
- [x] 在前端脚本新增分公司图表初始化、时间筛选联动、指标切换和 resize/theme 刷新逻辑。
- [x] 补充样式并运行定向测试、Python 编译检查和 JavaScript 语法检查。
- [x] 分公司页新增具体故障列表，复用物理故障页明细展示、筛选条件 badge、局部 KPI 和清除过滤交互。
- [x] 分公司页上方卡片和图表点击后仅过滤六省分公司明细，不影响物理故障页明细状态。
- [x] 补充分公司明细源码级回归测试并运行定向验证。
- [x] 为分公司页光缆中断情况补齐超时率和重复中断两个指标。
- [x] 为分公司页总体情况、光缆中断情况各指标增加与上周期一致的增减指示。
- [x] 运行分公司统计定向测试、Python 编译检查和 JavaScript 语法检查。

## 2026-06-01 分公司绩效考核卡片
- [x] 增加源码级回归测试，锁定分公司统计 payload 返回六省 `performance_cards`、责任/全量双口径得分和扣分项。
- [x] 在统计后端新增分公司绩效卡片聚合和评分 helper，复用六省光缆中断数据与百公里归一化口径。
- [x] 在分公司 Tab 顶部新增六省绩效考核卡片容器，并渲染责任得分、全量得分、核心 KPI、扣分解释和原因 TOP。
- [x] 接入卡片点击过滤，复用现有分公司明细下钻状态。
- [x] 补充卡片样式并运行定向测试、Python 编译检查和 JavaScript 语法检查。

## 2026-06-02 分公司绩效评分独立标签页
- [x] 增加源码级回归测试，锁定新增“分公司绩效评分”Tab，原分公司 Tab 改名为“分公司（省份）故障”。
- [x] 将六省绩效考核卡片从分公司故障 Tab 移到新的绩效评分 Tab。
- [x] 调整前端 Tab 切换加载逻辑，确保绩效评分 Tab 打开时加载并渲染卡片。
- [x] 运行分公司统计定向测试、JavaScript 语法检查和模板相关回归测试。

## 2026-06-02 分公司绩效评分卡片大版式
- [x] 增加源码级回归测试，锁定截图版式所需的公司图标、大分数、/100、胶囊等级、指标图标、扣分图标和原因面板类名。
- [x] 重构分公司绩效卡片渲染结构，按截图实现顶部身份区、右侧评分区、2x3 指标矩阵、责任扣分和原因条。
- [x] 重写卡片 CSS，确保单卡大版式、圆形图标、分隔线、红色扣分态和响应式布局符合截图。
- [x] 运行分公司统计定向测试、JavaScript 语法检查和 CSS/空白检查。

## 2026-06-02 分公司绩效卡片运行月历和中断日历
- [x] 增加源码级回归测试，锁定分公司绩效卡片 payload 包含 `monthly_stats`、`interrupt_calendar`、`interrupt_calendar_full`。
- [x] 在分公司绩效卡片下方复用业务卡片的运行月历图表和近三个月中断日历，不渲染月份 SLA 表格。
- [x] 为分公司绩效卡片初始化月历图表和中断日历展开交互。
- [x] 运行分公司统计测试、业务卡片资源测试、JavaScript 语法检查和空白检查。

## 2026-06-02 中断日历默认月份范围
- [x] 将默认中断日历范围从近三个月调整为当前选择月份向前六个月。
- [x] 同步业务卡片、分公司绩效卡片和指标说明中的近六个月文案。
- [x] 运行分公司统计测试、业务卡片资源测试、Python 编译、JavaScript 语法检查和空白检查。
## 2026-06-03 修复重保迁移导入 comments 字段
- [x] 增加源码级回归测试，锁定 `HeavyDuty` 模型提供 `comments` 字段并与表单、过滤器、API 引用一致。
- [x] 在 `HeavyDuty` 模型与迁移中补齐 `comments` 文本字段，解决 `migrate netbox_otnfaults` 导入表单时报 Unknown field。
- [x] 运行重保定向测试和 Python 编译检查验证变更。

## 2026-06-03 态势大屏运行看板改造
- [x] 更新大屏故障数据口径：仅展示 `fault_status='processing'` 的正在发生故障，不再输出已关闭故障热力图。
- [x] 新增大屏割接数据段，展示未来窗口内申请中/待实施割接，并纳入综合事件队列。
- [x] 将重保限定为文字展示：保留当前生效重保滚动，不新增地图图层。
- [x] 重构大屏左右面板：左侧保留网络健康度数字和三项核心数量，右侧改为综合事件队列与焦点详情。
- [x] 在左侧运行看板保留 24H 故障趋势图，继续使用后端 `trend_24h` 数据刷新。
- [x] 取消左侧展示口径描述卡片，减少文字说明占屏。
- [x] 调整前端渲染逻辑、样式和源码级回归测试，验证割接/重保/故障三类事件展示。

## 2026-06-04 测试语义化改造
- [x] 将旧重保 banner 测试改为验证当前重保信息条和事件队列接线。
- [x] 将旧大屏运行指标测试改为验证核心看板与辅助指标职责边界。
- [x] 将统计页精确 HTML 快照测试改为结构/语义断言，并移除对应旧布局 skip。
- [x] 运行相关定向测试验证变更。
## 2026-06-04 修复态势大屏 24H 故障趋势响应式比例
- [x] 增加源码级回归测试，锁定趋势图使用 CSS 比例和 JS 容器尺寸自适应。
...
## 2026-06-12 故障统计模块：子公司绩效卡片对齐裸纤业务卡片
- [x] 增加源码级回归测试，锁定子公司绩效卡片复用裸纤业务卡片的标题、卡片网格、年度区块、运行月历和中断日历 CSS 结构。
- [x] 调整 `statistics_dashboard.js`，让子公司绩效卡片 DOM 使用 `service-strip-card` / `service-annual-summary` / `service-runtime-calendar` / `service-interrupt-calendar` 体系。
- [x] 清理 `statistics_dashboard.css` 中子公司绩效卡片的重复仿制样式，只保留必要的年度指标列数和点击态覆盖。
- [x] 运行子公司统计定向测试、CSS/JS 源码约束和语法检查。
## 2026-06-12 故障统计模块：子公司绩效年度累计标题补齐
- [x] 增加源码级回归测试，锁定子公司绩效年度区块显示“年度累计（YYYY年）”提示。
- [x] 调整 `statistics_dashboard.js` 中子公司绩效年度指标标题，复用裸纤业务卡片年度累计文案结构。
- [x] 运行子公司统计定向测试和 JS 语法检查。
## 2026-06-12 故障统计模块：子公司绩效年度累计标题位置调整
- [x] 增加源码级回归测试，锁定“年度累计（YYYY年）”只在子公司绩效卡片年度区第一行显示一次。
- [x] 调整 `statistics_dashboard.js`，将年度累计标题从各指标分组上移到卡片年度区首行，分组标题仅保留业务类型名称。
- [x] 运行子公司统计定向测试和 JS 语法检查。
## 2026-06-12 故障统计模块：子公司绩效标题栏年度累计
- [x] 增加源码级回归测试，锁定“年度累计（YYYY年）”移动到子公司绩效卡片标题栏分公司名称后，并保留深色标题栏内图标与字体样式。
- [x] 调整 `statistics_dashboard.js`，将年度累计提示从正文年度区块移入 `service-strip-card-title`。
- [x] 调整 `statistics_dashboard.css`，为标题栏内的年度累计图标和文字提供深色背景适配样式。
- [x] 运行子公司统计定向测试和 JS 语法检查。
## 2026-06-12 故障统计模块：子公司绩效运行月历单位标识
- [x] 增加源码级回归测试，锁定子公司绩效运行月历柱状图标签不再逐个拼接 `次/千公里`。
- [x] 调整 `statistics_dashboard.js`，将千公里单位集中放到运行月历右侧单位标识，柱状图标签仅显示数值。
- [x] 运行子公司统计定向测试和 JS 语法检查。
## 2026-06-12 故障统计模块：子公司绩效运行月历单位位置微调
- [x] 增加源码级回归测试，锁定计数单位使用右侧中部位置，避免与 x 轴月份标签重合。
- [x] 调整 `statistics_dashboard.js` 中计数轴单位的 `nameLocation`、`nameGap` 和右侧边距。
- [x] 运行子公司统计定向测试和 JS 语法检查。
## 2026-06-12 故障统计模块：子公司绩效运行月历单位右侧对齐
- [x] 增加源码级回归测试，锁定计数单位不使用额外 `offset`，避免相对右上角“时”横向偏移。
- [x] 调整 `statistics_dashboard.js` 中计数轴单位的右侧边距与文字对齐方式，使其与“时”保持同一右侧轴线。
- [x] 运行子公司统计定向测试和 JS 语法检查。
## 2026-06-14 故障统计模块：子公司绩效运行月历计数单位位置与颜色微调
- [x] 调整 `statistics_dashboard.js`，将运行月历柱状图（故障数）单位标识（如次/千公里）的颜色修改为与上方折线图（“时”单位）一致的 `#078087`。
- [x] 将运行月历柱状图单位标识的位置通过增加 top padding 往下移动，避免其过于靠上（从 `padding: [48, 0, 0, 0]` 调整为 `padding: [78, 0, 0, 0]`）。
- [x] 修改 `tests/test_statistics_branch_company.py` 里的静态断言，将对应的 padding 限制更新为 `[78, 0, 0, 0]`，确保本地单元测试通过。

## 2026-06-14 故障统计模块：运行月历未来月份折线颜色淡化
- [x] 调整 `statistics_dashboard.js`，通过 ECharts 的 `visualMap` 功能，将运行月历（子公司绩效图和业务卡片图）在当前日期（今年未来月份）之后的数据线和数据点展示为浅灰色（`#cbd5e1`）。
- [x] 配合 `visualMap` 的分段着色，将折线的 `areaStyle` 颜色改为继承自折线颜色并保持 `opacity: 0.08`，使未来的填充背景同样自动变为浅灰色。
- [x] 运行子公司统计与概览定向测试验证，确保所有断言仍然完美通过。
...
## 2026-07-08 菜单分组按方案A重组
- [x] 增加导航菜单源码级回归测试，锁定新的单一级菜单分组和入口顺序。
- [x] 调整 `navigation.py` 中的 `PluginMenu` 分组为运行态势、故障处置、割接管理、保障任务、业务资源、网络资源。
- [x] 运行导航菜单定向测试和 Python 编译检查。

## 2026-07-29 外购合同付款区间筛选关联故障
- [x] 读取付款行起止日期并按故障发生日期做闭区间筛选。
- [x] 在关联故障卡片展示当前筛选区间并提供清除入口。
- [x] 增加定向回归测试并完成 Python 编译检查。
## 2026-07-29 外购合同关联故障局部筛选性能优化
- [x] 抽取关联故障查询与表格上下文构建逻辑，并应用对象权限限制。
- [x] 增加仅渲染关联故障卡片的轻量 HTMX 端点。
- [x] 将付款筛选与清除筛选按钮切换到轻量端点，避免重新生成完整合同详情页。
- [x] 增加定向回归测试并完成 Python 编译检查。
## 2026-07-29 外购合同付款标签联动筛选故障
- [x] 付款筛选请求携带当前付款记录的标签 ID。
- [x] 关联故障查询在日期区间基础上增加相同标签匹配，并避免重复记录。
- [x] 在筛选状态中提示已启用付款标签匹配。
- [x] 增加定向回归测试并完成 Python 编译检查。

## 2026-07-30 态势大屏次要文字对比度优化
- [x] 增加大屏样式回归测试，锁定割接时间、事件摘要和焦点详情标签的高对比度颜色。
- [x] 调整大屏信息性次要文字颜色，提升深色背景下的远距离可读性。
- [x] 运行大屏定向测试并检查样式变更范围。

## 2026-07-30 割接通报业务与站点格式优化
- [x] 更新通报格式回归测试，锁定“业务名称：A站点→Z站点”结构。
- [x] 调整割接通报业务行格式并保留方括号。
- [x] 运行今明割接小组件定向测试与 Python 编译检查。

## 2026-07-30 今明割接通报按钮窄宽换行
...
- [x] 将无关联业务的故障影响文案从 `影响：-线路` 调整为 `影响：线路组网`。
- [x] 运行故障文本定向测试、相关回归、语法及差异检查。

## 2026-08-05 交接班生成前检查逾期待实施割接

- [x] 新增权限受限的逾期待实施割接查询、字段映射和插件只读检查视图。
- [x] 在警示窗口列出割接编号、计划时间、省份、类型、AZ 端和地点，并提供新窗口编辑链接。
- [x] 实现首次无记录直达、跳过、再次检查、复查刷新、复查为空后 5 秒倒计时和错误恢复。
- [x] 运行交接班及相关仪表盘回归、Python/JavaScript 语法和差异检查（浏览器因 localhost:50523 未启动而未执行）。

## 2026-08-05 交接班核对完成倒计时 UI 改造

- [x] 复查通过后保留割接列表，并增加逐行绿色对勾、浅绿色背景和顶部成功状态。
- [x] 将倒计时放入可点击的绿色按钮，增加进度条，并支持点击立即进入。
- [x] 统一按钮点击与计时器到期路径，防止重复生成，并在下次检查时恢复初始状态。
- [x] 运行相关回归、Python/JavaScript 语法和差异检查（浏览器因 localhost:50523 不可用而未执行）。
## 2026-08-04 复制本组增加预告标题

- [x] 增加失败回归测试，锁定“预告标题 + 空行 + 本组正文”的复制格式。
- [x] 调整“复制本组”逻辑，不包含业务分组标题且不影响复制全部。
- [x] 运行今明割接小组件定向测试、Python 编译检查与差异检查。

## 2026-08-17 子公司统计增加北京京宽排除项
- [x] 增加回归断言，锁定“北京京宽网络科技有限公司”属于子公司统计排除单位。
- [x] 将该公司加入 `EXCLUDED_HANDLING_UNITS` 统一排除集合。
- [x] 运行子公司统计定向测试和 Python 语法检查。
