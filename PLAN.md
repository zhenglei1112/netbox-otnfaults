...
- [ ] 移除物理故障、子公司、裸纤业务和电路业务明细分页 UI 与前端分页状态。
- [ ] 调整故障明细和业务明细接口，一次返回全部筛选结果。
- [ ] 恢复物理故障和子公司完整明细缓存、局部统计及按时间/按重复排序。
- [ ] 故障明细补充与当前周期关联的前序重复故障，并保持子公司范围过滤一致。
- [ ] 运行统计定向测试、业务排序断言、JavaScript 和 Python 语法检查。
