...
- [ ] 增加回归测试，锁定默认仅返回发生故障业务、显式参数按需返回全部裸纤业务。
- [ ] 后端业务统计接口支持 `include_all_bare_fiber=1`，仅在显式请求时初始化全部裸纤业务。
- [ ] 前端切换“全部业务”时按需请求全量数据，并在同一统计条件下复用缓存。
- [ ] 运行统计定向测试、JavaScript 语法检查和 Python 编译检查。
