...
class StatisticsCableBreakOverviewTestCase(unittest.TestCase):
    ...
    def test_statistics_cable_break_map_mode_is_isolated_from_fault_mode_controls(self) -> None:
        ...
        self.assertIn("'plugin_file': 'statistics_cable_break_mode.js?v=4'", mode_block)
    ...
    def test_statistics_cable_break_map_plugin_handles_iframe_and_skipped_count(self) -> None:
        ...
        self.assertIn("this._addDefaultMarkers();", source)
        self.assertIn("_addDefaultMarkers()", source)
        self.assertIn("_getMarkerColor(feature)", source)
        self.assertIn("const statusKey = properties.statusKey || properties.status_key || 'processing';", source)
        self.assertIn("properties.statusColorHex ||", source)
        self.assertIn("FAULT_STATUS_COLORS[statusKey]", source)
        self.assertIn("new maplibregl.Marker({ color: this._getMarkerColor(feature) })", source)
        self.assertIn("marker.getElement().addEventListener(\"mouseenter\"", source)
        self.assertIn("marker.getElement().addEventListener(\"mouseleave\"", source)
        ...
        self.assertIn("marker.getElement().addEventListener(\"click\"", source)
        self.assertNotIn("_loadStandardMarkerSymbol", source)
        self.assertNotIn("_loadFaultIcons", source)
        self.assertNotIn("addImage", source)
        self.assertNotIn('"icon-image"', source)
