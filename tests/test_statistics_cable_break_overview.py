...
class StatisticsCableBreakOverviewTestCase(unittest.TestCase):
    ...
    def test_overall_total_excludes_degradation_and_jitter_with_other_card_data(self) -> None:
        ...
        self.assertIn("_apply_physical_province_filter(", source)
    ...
    def test_overall_other_card_renders_degradation_jitter_and_suspended_faults(self) -> None:
        ...
        self.assertIn("renderOverallOtherSummary(data.other_overview, data.prev_other_overview, data.yoy_other_overview);", source)
    ...
    def test_reason_top3_uses_unreported_rectification_display_alias(self) -> None:
        ...
        self.assertIn("statistics_dashboard.js' %}?v=47", template)
    ...
    def test_service_cards_render_existing_metrics_in_strip_card_layout(self) -> None:
        ...
        self.assertIn("currentBareFiberServices = getServicesByType(services, '裸纤业务');", source)
        self.assertIn("renderBareFiberServiceCards();", source)
    ...
    def test_bare_fiber_service_tab_exposes_card_scope_toggle(self) -> None:
        template = TEMPLATE_PATH.read_text(encoding="utf-8")
        source = JS_PATH.read_text(encoding="utf-8")
        css = CSS_PATH.read_text(encoding="utf-8")

        self.assertIn('id="bare-fiber-service-card-scope-toggle"', template)
        self.assertIn('aria-label="裸纤业务卡片显示范围"', template)
        self.assertIn('name="bareFiberServiceCardScope"', template)
        self.assertIn('id="bare-fiber-scope-faulted"', template)
        self.assertIn('value="faulted"', template)
        self.assertIn('id="bare-fiber-scope-all"', template)
        self.assertIn('value="all"', template)
        self.assertIn("发生故障业务", template)
        self.assertIn("全部业务", template)
        self.assertNotIn("只显示时间范围内有故障的业务", template)
        self.assertNotIn("显示全部业务", template)
        toolbar_block = template.split('<div class="d-flex gx-2 align-items-center filter-controls">', 1)[1].split('id="statistics-metric-help-btn"', 1)[0]
        self.assertIn('id="bare-fiber-service-card-scope-toggle"', toolbar_block)

        self.assertIn("let currentBareFiberServices = [];", source)
        self.assertIn("let bareFiberServiceCardScope = 'faulted';", source)
        self.assertIn("let bareFiberAllServicesLoaded = false;", source)
        self.assertIn("async function loadServiceData({ includeAllBareFiber = false } = {})", source)
        ...
        self.assertIn("let serviceDataRequestSequence = 0;", source)
        self.assertIn("if (requestSequence !== serviceDataRequestSequence) return;", source)
        self.assertIn("function serviceHasCurrentPeriodFaults(svc)", source)
        self.assertIn("return Number(svc && svc.count || 0) > 0;", source)
        self.assertIn("function getVisibleBareFiberServices()", source)
        self.assertIn("bareFiberServiceCardScope === 'all'", source)
        self.assertIn("return currentBareFiberServices.filter(serviceHasCurrentPeriodFaults);", source)
        self.assertIn("function renderBareFiberServiceCards()", source)
        self.assertIn("renderServiceCards(getVisibleBareFiberServices(), 'service-cards-container', '裸纤业务');", source)
        self.assertIn("currentBareFiberServices = getServicesByType(services, '裸纤业务');", source)
        self.assertIn("renderBareFiberServiceCards();", source)
        self.assertNotIn("renderServiceCards(getServicesByType(services, '裸纤业务'), 'service-cards-container', '裸纤业务');", source)
        self.assertIn("function syncBareFiberServiceCardScopeToggle()", source)
        self.assertIn("scopeToggle.classList.toggle('d-none', activeTabId !== 'tab-service-btn');", source)
        self.assertIn("bareFiberServiceCardScopeInputs.forEach(input => {", source)
        self.assertIn("input.addEventListener('change', () => {", source)
        self.assertIn("bareFiberServiceCardScope = input.value === 'all' ? 'all' : 'faulted';", source)
        self.assertIn("if (bareFiberServiceCardScope === 'all' && !bareFiberAllServicesLoaded)", source)
        self.assertIn("loadServiceData({ includeAllBareFiber: true });", source)
        self.assertIn("includeAllBareFiber: activeTab.id === 'tab-service-btn' && bareFiberServiceCardScope === 'all'", source)
        self.assertIn("includeAllBareFiber: event.target.id === 'tab-service-btn' && bareFiberServiceCardScope === 'all'", source)
        self.assertIn('data-service-type="${escapeHtml(card.serviceType || \'\')}"', source)
        ...
        self.assertIn(".bare-fiber-service-card-scope-toggle .btn", css)
        self.assertIn(".bare-fiber-service-card-scope-toggle .btn-check:checked + .btn", css)
        self.assertIn("background: #008a7a;", css)
        self.assertIn("color: #ffffff;", css)
        self.assertIn("border-color: #008a7a;", css)
        scope_button_block = css.split(".bare-fiber-service-card-scope-toggle .btn {", 1)[1].split("}", 1)[0]
        self.assertNotIn("min-height:", scope_button_block)
        self.assertNotIn("height:", scope_button_block)
        self.assertNotIn("font-weight:", scope_button_block)
        self.assertNotIn("font-size:", scope_button_block)
    ...
    def test_service_cards_render_current_period_category_breakdown(self) -> None:
        ...
        self.assertIn("calendar_full_start = calendar_full_months[0]['start']", views_source)
        self.assertIn("calendar_full_end = calendar_full_months[-1]['end']", views_source)
        self.assertIn("calendar_query_start = min(calendar_start, calendar_full_start)", views_source)
        self.assertIn("calendar_query_end = max(calendar_end, calendar_full_end)", views_source)
        ...
        self.assertIn("'interrupt_calendar_full': {", views_source)
        self.assertIn("for month_info in calendar_full_months", views_source)
        self.assertIn("monthly_stats = {month: {'count': 0, 'duration': 0.0, 'intervals': []} for month in range(1, 13)}", views_source)
        ...
        self.assertIn("stats['interrupt_calendar_full'][calendar_key][calendar_day.day] += 1", views_source)
        self.assertIn("monthly_stats_payload = [", views_source)
        self.assertIn("interrupt_calendar_payload = [", views_source)
        self.assertIn("interrupt_calendar_full_payload = [", views_source)
        ...
        self.assertIn("'interrupt_calendar_full': interrupt_calendar_full_payload,", views_source)
    ...
    def test_service_cards_render_runtime_calendar_chart(self) -> None:
        ...
        self.assertIn("function renderServiceRuntimeCalendar(svc)", source)
        self.assertIn("function renderServiceRuntimeCalendarSlaTable(svc)", source)
        ...
        runtime_block = source.split("function renderServiceRuntimeCalendar(svc)", 1)[1].split("function getInterruptCalendarLevel", 1)[0]
        self.assertLess(
            runtime_block.index("${renderServiceRuntimeCalendarSlaTable(svc)}"),
            runtime_block.index('<div class="service-runtime-calendar-chart" aria-label="本年业务故障月度统计"></div>'),
        )
        self.assertIn('<div class="service-runtime-calendar-chart" aria-label="本年业务故障月度统计"></div>', source)
        self.assertIn('<div class="service-runtime-calendar-sla-grid" aria-label="本年各月SLA情况">', source)
        self.assertIn("const firstHalf = monthlyStats.slice(0, 6);", source)
        self.assertIn("const secondHalf = monthlyStats.slice(6, 12);", source)
        self.assertIn("renderServiceRuntimeCalendarSlaRow(firstHalf, annualYear, currentYear, currentMonth)", source)
        self.assertIn("renderServiceRuntimeCalendarSlaRow(secondHalf, annualYear, currentYear, currentMonth)", source)
        self.assertIn("const annualYear = Number(svc.annual_summary && svc.annual_summary.year || new Date().getFullYear());", source)
        self.assertIn("const currentYear = new Date().getFullYear();", source)
        self.assertIn("const currentMonth = new Date().getMonth() + 1;", source)
        self.assertIn("const showSla = annualYear < currentYear || (annualYear === currentYear && monthNumber <= currentMonth);", source)
        ...
        self.assertIn("function renderServiceInterruptCalendarMonthGrid(months, maxCount)", source)
        ...
        self.assertIn('service-interrupt-calendar-months--default', source)
        self.assertIn('service-interrupt-calendar-months--expanded d-none', source)
        self.assertIn('class="service-interrupt-calendar-toggle"', source)
        self.assertIn('aria-label="展开本年中断日历"', source)
        self.assertIn('mdi-arrow-expand-all', source)
        self.assertIn("function initServiceInterruptCalendarToggles(container)", source)
        self.assertIn("event.stopPropagation();", source)
        self.assertIn("calendar.classList.toggle('service-interrupt-calendar--expanded', expanded);", source)
        self.assertIn("defaultMonths.classList.toggle('d-none', expanded);", source)
        self.assertIn("expandedMonths.classList.toggle('d-none', !expanded);", source)
        self.assertIn("icon.className = expanded ? 'mdi mdi-arrow-collapse-all' : 'mdi mdi-arrow-expand-all';", source)
        ...
        self.assertIn("let durationAxisMin, durationAxisMax;", source)
        self.assertIn("durationAxisMin = -Math.max(1, Math.ceil(maxDuration * 1.5));", source)
        self.assertIn("durationAxisMax = Math.max(1, Math.ceil(maxDuration * 1.15));", source)
        ...
        self.assertIn("name: '时'", source)
        self.assertIn("position: 'right'", source)
        self.assertIn("nameLocation: 'end'", source)
        self.assertIn("nameGap: 2", source)
        self.assertIn("return value > 0 ? formatCardMetricValue(value) : '';", source)
        self.assertNotIn("return value > 0 ? `${formatCardMetricValue(value)}时` : '';", source)
        ...
        self.assertIn(".service-runtime-calendar-sla-grid {", css)
        self.assertIn("grid-template-columns: repeat(6, minmax(0, 1fr));", css)
        self.assertIn("border-top: 1px solid rgba(154, 168, 186, 0.45);", css)
        self.assertIn("border-left: 1px solid rgba(154, 168, 186, 0.45);", css)
        self.assertIn(".service-runtime-calendar-sla-cell {", css)
        sla_cell_block = css.split(".service-runtime-calendar-sla-cell {", 1)[1].split("}", 1)[0]
        self.assertIn("border-right: 1px solid rgba(154, 168, 186, 0.45);", css)
        self.assertIn("border-bottom: 1px solid rgba(154, 168, 186, 0.45);", css)
        self.assertNotIn("border-radius:", sla_cell_block)
        self.assertIn(".service-runtime-calendar-sla-month {", css)
        self.assertIn(".service-runtime-calendar-sla-value {", css)
        ...
        self.assertIn(".service-interrupt-calendar-months--expanded {", css)
        self.assertIn("grid-template-columns: repeat(auto-fill, minmax(5.4rem, 1fr));", css)
        ...
        self.assertIn(".service-interrupt-calendar-toggle {", css)
        self.assertIn("position: absolute;", css)
        self.assertIn("right: 0.72rem;", css)
        self.assertIn("bottom: 0.55rem;", css)
        self.assertIn(".service-interrupt-calendar--expanded .service-interrupt-calendar-toggle {", css)
        self.assertIn("function getMaxServiceInterruptCalendarCount(services)", source)
        self.assertIn("services.flatMap(svc =>", source)
        self.assertIn("const calendars = [svc.interrupt_calendar, svc.interrupt_calendar_full].filter(Array.isArray);", source)
    ...
    def test_statistics_dashboard_contains_cable_break_map_modal_with_loading(self) -> None:
        ...
        self.assertIn("cableBreakMapIframe.src = `${window.STATISTICS_CABLE_BREAK_MAP_URL}?modal=true&${buildTimeParams()}${buildPhysicalProvinceParams()}`;", source)
    ...
    def test_statistics_cable_break_map_uses_in_map_quick_filters_without_iframe_reload(self) -> None:
        ...
        self.assertIn("cableBreakMapIframe.src = `${window.STATISTICS_CABLE_BREAK_MAP_URL}?modal=true&${buildTimeParams()}${buildPhysicalProvinceParams()}`;", dashboard_source)
    ...
    def test_overall_physical_fault_card_has_daily_category_line_chart(self) -> None:
        ...
        self.assertIn("physical_duration_boxplot_faults = list(_apply_physical_province_filter(", views_source)
        self.assertIn("global_cable_break_faults = list(get_cable_break_base_queryset(start_date, end_date))", views_source)
