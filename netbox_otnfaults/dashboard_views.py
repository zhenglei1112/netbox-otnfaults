    ...
    def get(self, request) -> JsonResponse:
        ...
        for fault in active_faults_qs:
            ...
            active_faults.append({
                ...
                'occurrence_time': timezone.localtime(fault.fault_occurrence_time).isoformat() if fault.fault_occurrence_time else None,
                'recovery_time': timezone.localtime(fault.fault_recovery_time).isoformat() if fault.fault_recovery_time else None,
                'duration': duration_text,
                'dispatch_time': timezone.localtime(fault.dispatch_time).isoformat() if fault.dispatch_time else None,
                'departure_time': timezone.localtime(fault.departure_time).isoformat() if fault.departure_time else None,
                'arrival_time': timezone.localtime(fault.arrival_time).isoformat() if fault.arrival_time else None,
                'repair_time': timezone.localtime(fault.repair_time).isoformat() if fault.repair_time else None,
        ...
        for fault in recent_faults:
            ...
            ticker_events.append({
                ...
                'site_a': fault.interruption_location_a.name if fault.interruption_location_a else '',
                'sites_z': z_sites,
                'time': timezone.localtime(fault.fault_occurrence_time).strftime('%m-%d %H:%M') if fault.fault_occurrence_time else '',
                'duration': duration_text,
        ...
        for hd in active_heavy_duties:
            heavy_duties_data.append({
                ...
                'start_time_display': timezone.localtime(hd.start_time).strftime('%m月%d日 %H:%M') if hd.start_time else '',
                'end_time_display': timezone.localtime(hd.end_time).strftime('%m月%d日 %H:%M') if hd.end_time else '',
